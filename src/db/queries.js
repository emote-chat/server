...
module.exports = {
    ...
    findChatsByUserId: new PS('find-users-by-chat-id', "SELECT chats_id AS id, name, json_agg(json_build_object('id', users_id, 'display_name', display_name)) as users FROM users_chats INNER JOIN chats ON users_chats.chats_id=chats.id INNER JOIN users ON users_chats.users_id=users.id WHERE users_chats.chats_id IN (SELECT chats_id FROM users_chats WHERE users_id = $1) GROUP BY chats_id, chats.name"),
    // users_chats
